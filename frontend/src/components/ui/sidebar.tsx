...
function SidebarMenuButton({
  ...
  const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    // ejecuta el onClick pasado desde props primero
    onClick?.(event)

    // busca el botón de acción dentro del mismo `sidebar-menu-item` y dispara su click si existe
    try {
      const current = event.currentTarget as HTMLElement
      const menuItem = current.closest('[data-slot="sidebar-menu-item"]') as HTMLElement | null
      const action = menuItem?.querySelector<HTMLElement>('[data-slot="sidebar-menu-action"]')
      if (action) {
        action.click()
      }
    } catch {
      // ignora cualquier error por seguridad
    }
  }
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground h-7 gap-2 rounded-md px-2 focus-visible:ring-2 data-[size=md]:text-sm data-[size=sm]:text-xs [&>svg]:size-4 flex min-w-0 -translate-x-px w-full items-center overflow-hidden outline-hidden group-data-[collapsible=icon]:hidden disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:shrink-0",
        className
      )}
      onClick={handleClick}
      {...props}
